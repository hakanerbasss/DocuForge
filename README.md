...
- ✅ Uploadable documentary closing image: an alternative to the AI-generated closing shot (above) for anyone who wants one consistent, hand-designed ending (e.g. made in ChatGPT/DALL-E) across every video instead of a fresh one each time. Upload it once in `/settings`; `RenderService` swaps it in for the last scene's visual on any `documentary` project from then on -- narration audio for that scene is untouched, only the image changes. Not configured by default, so nothing changes until you upload one; "Değiştir" replaces it any time

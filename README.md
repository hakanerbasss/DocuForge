...
- ✅ Background music: if `background_music_enabled` is set, it's looped/trimmed to the video length, mixed in below narration volume, and faded out. Source is either a local file (`projects/<slug>/music/` or an explicit `music_track` path) or a `music_provider`: `local` (default), `jamendo` (royalty-free search API) or `mubert` (AI-generated music from a mood prompt derived from `content_type`) — selectable per-project from the wizard or the render step's "Yeniden Üret" form; API keys go in `/settings`
...
- ✅ Thumbnail: if `thumbnail_enabled` is set, DocuForge always produces **4 genuinely different high-CTR compositions** (`thumbnail_1..4.png`) — `split_contrast`, `mystery_focus`, `documentary_cinematic`, `breaking_discovery` — each with its own crop, color grade, gradient/vignette/blur treatment, and text placement, never just the same layout with the text moved. Backgrounds come from the DALL-E provider (per-template prompt built from an SEO creative brief) when `OPENAI_API_KEY` is configured, falling back per-template to the strongest real scene frame (auto-picked via a Pillow edge/contrast heuristic) otherwise. On-image text is always a short 3-6 word hook (never the full title), drawn locally with Pillow (bold font, stroke, shadow, auto-shrink-to-fit instead of truncating) so Turkish diacritics render correctly. One variant becomes the canonical `thumbnail.jpg` by default (`content_type`-based), and the project page shows all 4 side by side to download or pick from

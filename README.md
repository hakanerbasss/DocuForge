...
- ✅ Thumbnail: if `thumbnail_enabled` is set, a 1280x720 YouTube thumbnail is generated from a scene frame (plus a 1080x1920 cover for shorts/vertical projects) — pure FFmpeg, no extra dependency. Text is the SEO agent's curiosity-driven title suggestion (falls back to the raw project title), and the visual style rotates round-robin across 4 templates (bottom banner, top banner, side color stripe, bold black-outline text) so a channel doesn't publish the same cookie-cutter cover on every video

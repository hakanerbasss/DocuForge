...
| `ThumbnailService` | Optional. Builds an SEO creative brief (`thumbnail_hook`/`main_subject`/`emotional_trigger`/`visual_contrast`/`avoid_elements` from `seo.json`, with fallbacks). Picks the strongest real scene frame via a Pillow edge/contrast heuristic (`_score_frame`) across all scenes as a fallback background source. For each of 4 templates (`split_contrast`, `mystery_focus`, `documentary_cinematic`, `breaking_discovery`) it builds a template-specific prompt and generates a background through the `dalle` image provider when `OPENAI_API_KEY` is configured, falling back to the real scene frame per-template on any failure. Composites the final image with Pillow (gradients, blur/vignette, dutch-angle rotation, split color-grading, stroke+shadow text, auto-shrink-to-fit wrapping) -- text is never the full title, always a short hook. Writes `thumbnail_1.png`..`thumbnail_4.png`, copies one (chosen by `content_type`) to the canonical `thumbnail.jpg` (+ `thumbnail_vertical.jpg` for shorts/vertical, recomposited from the same chosen template's background), and records the pick as `thumbnail_selected` in `project.json`. The project page shows all 4 variants; `POST /api/projects/{slug}/thumbnail/select` lets the user swap which one is canonical. |

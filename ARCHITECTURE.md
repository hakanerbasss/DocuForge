...
| `VoiceService` | Synthesizes each scene's narration audio via the voice provider registry (eSpeak / Piper / Supertonic / XTTS), using `voice_provider`, `voice_name`, `voice_speed` from `project.json`. Runs each scene's text through `clean_tts_text()` (`app/utils/tr_tts_normalize.py`) immediately before calling `provider.synthesize()` — this is the single chokepoint all four providers go through, so Turkish number/date/abbreviation normalization applies uniformly regardless of which engine is selected. Writes `audio/manifest.json` with per-scene duration. |
